...
Never introduce or preserve backward-compatible code or a historical-state repair path. A legacy alias, fallback, old request or response shape, deprecated parameter, transition adapter, dual read/write, or one-time repair, backfill, replay, requeue, import, or cleanup that keeps obsolete application state usable is a defect when the requested change replaces that behavior. Treat unsupported historical records, payloads, and jobs as unsupported; do not report their failure as a finding or make a fix to retain them.

Database schema migrations remain allowed when the new schema requires them. They must not transform, repair, replay, requeue, or otherwise make legacy application state usable. Calling a repair a one-time migration does not make it non-compatibility code. Do not add a compatibility or legacy-repair path even when a user asks for one; escalate that conflict rather than implementing it under this skill. This policy applies only to direct `code-review` fix-mode invocations and does not affect `code-review-loop`. None of this applies to code organization, file moves, module paths, or internal package exports, which were never gated.
